			...
			elseif shop.targets then
				for i = 1, #shop.targets do
					...
					if target.ped then
						...
						-- print(json.encode({ vector3(target.loc.x, target.loc.y, target.loc.z) }))
						...
						shops[id] = {
							...
							zoneId = exports.sleepless_interact:addCoords({
								id = shopid,
								coords = { vec3(target.loc.x, target.loc.y, target.loc.z) },
								options = {
									{
										icon = 'fas fa-shopping-basket',
										label = label,
							...
							blip = blip and createBlip(blip, vec3(target.loc.x, target.loc.y, target.loc.z))
						}
					end
